export default async function handler(req, res) {
  try {
    ...
    // Ejemplos naturales y comprensibles por grado.
...
function sentenceGuide`;
    ...
    html = html.replace('<div class="txt"><b>Observa cómo se usa la palabra dentro de una oración completa:</b></div>','<div class="txt"><b>Lee este ejemplo. La oración cuenta una situación que puedes imaginar y entender:</b></div>');
    html = html.replace('<span>No respondas con una sola palabra. La respuesta debe ser una oración completa que tenga sentido.</span>','<span>No copies el ejemplo. Inventa otra situación y escribe una oración completa que tenga sentido.</span>');

    html = html.replace('const specific={',`const specific={
  "guerra":{3:["Piensa en lo que puede pasar durante una guerra. ¿Cómo afecta a las personas?",["guerra","familias"],"Escribe una idea sencilla sobre una consecuencia de la guerra."]},
  "cansado":{
   ...
   6:["Describe una causa y una consecuencia usando cansado.",["cansado","porque"],"Relaciona las dos ideas con claridad."]
    ...
    html = html.replace(/📓 <b>Todo se escribe en tu libreta\.<\/b>[\s\S]*?<\/div>/,'📓 <b>Todo se escribe en tu libreta.</b> Lee la explicación y el ejemplo; después inventa una oración diferente y continúa al siguiente reto.</div>');

    // Conservamos controles técnicos ocultos DENTRO del DOM antes de ejecutar el JS original.
    // Esto evita que el script se detenga al buscar photo/preview/verify/retake/status/feedback.
    html = html.replace(/<section class="step cameraCard">[\s\S]*?<\/section>/, `<section class="step mission" id="writeReminder">
      ...
      <div aria-hidden="true" style="display:none!important">
        <input id="photo" type="file"><img id="preview" alt=""><button id="verify" type="button"></button><button id="retake" type="button"></button><div id="status"></div><div id="feedback"></div>
      </div>
    </section>`);
    ...
    html += `<style>
      .cameraCard,.cameraZone,.aiNote{display:none!important}
    ...
    <script>
      document.addEventListener('DOMContentLoaded',()=>{
        ...
        if(n){
          n.disabled=false;
          // Reasignación defensiva: el botón siempre avanza aunque el JS heredado
          // haya sufrido una interrupción antes de registrar su manejador original.
          n.onclick=()=>{
            idx++;
            if(idx>=lessons.length) finish();
            else render();
          };
        }
      });
